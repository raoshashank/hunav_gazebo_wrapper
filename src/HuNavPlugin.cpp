...
namespace hunav
{
  ...
  void HuNavPlugin::Load(gazebo::physics::WorldPtr _world, sdf::ElementPtr _sdf)
  {
    ...
    hnav_->robotgest_sub = hnav_->rosnode->create_subscription<std_msgs::msg::UInt32>(
                          ...
                          std::bind(&HuNavPluginPrivate::robotGestCallback, hnav_.get(),
                                    std::placeholders::_1));
  ...
  bool HuNavPluginPrivate::InitializeRobot()
  {
    ...
    else
    {
      ...
      RCLCPP_INFO(rosnode->get_logger(), "Robot model %s found and INITIALIZED!!!!",
                   robotName.c_str());
      return true;
    }
  }
  ...
  void HuNavPluginPrivate::InitializeAgents()
  {
    ...
    // Call the computeAgentsService that returns the updated agents.
    ...
    if (result.wait_for(ms) == std::future_status::ready)
    {
      ...
      //updates "pedestrians" (used by gazebo) with the agents received from the service call
      for (auto agent : agents.agents)
      {
        ...
        ag.gesture = agent.gesture;
        
        pedestrians.push_back(ag); //update the pedestrians vector with agent updated from the service response
        RCLCPP_INFO(rosnode->get_logger(),
  ...
  /////////////////////Updates Gazebo's pedestrian position and velocity for current timestep(called by OnUpdate)////////////////////////////
      ...
      const hunav_msgs::msg::Agents &_agents)
  {
    ...
    {
      // RCLCPP_INFO(rosnode->get_logger(),
      //             "UpdateGazeboPedestrians. Gesture: %i ",
      //             a.gesture); 
      // auto model =
      ...
      for (unsigned int i = 0; i < pedestrians.size(); i++)
      {
        ...
        if (a.id == pedestrians[i].id)
        {
          ...
          //update gesture
          this->pedestrians[i].gesture = a.gesture;
        }
      }
  ...
  ////////////////////Called at every simulation timestep/////////////////////////////
  void HuNavPluginPrivate::OnUpdate(const gazebo::common::UpdateInfo &_info)
  {
    ...
    if (res == std::future_status::ready)
    {
      ...
      // update the Gazebo actors
      //auto updated_agents = result.get()->updated_agents;
      // for (auto a : updated_agents.agents){
      //   RCLCPP_INFO(rosnode->get_logger(),
      //             "OnUpdate. Gesture: %i from service",
      //             a.gesture); 
      // }
